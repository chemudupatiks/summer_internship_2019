...
def XY_scatter_plot(filename, x, x_time, y, y_time, radar = False, date = None, s_time = None, e_time = None, xlabel = None, ylabel = None, threshold = None ):    
    ...
    if date and s_time and e_time:
        ...
        if radar: 
            refl_st_index = np.where(x_time.astype(int) == start_time)[0][0]
            refl_end_index = np.where(x_time.astype(int) == end_time)[0][-1]
...
if n_args <= 1:
    ...
    XY_scatter_plot(filename, nevz, full_time, lower_gate_refl, times, date="03/09/2017", s_time = "14.23.11", e_time="14.24.38", xlabel = "Nevzorov LWC", ylabel = "nearest lower gate refl", radar = True) 
